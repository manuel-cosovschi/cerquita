...
| Auditar el plan como se audita el código                | Se revisaron ocho líneas marcadas `✅`. Una era falsa (el chequeo de accesibilidad no incluía la consola), una escondía un bug (el bloqueo tapaba en búsqueda y mapa pero servía la publicación por su id), y seis eran ciertas pero no las cubría nada. Un `✅` sin nada que lo vigile es una afirmación sobre el pasado, no una garantía: describe el día que se escribió, no el estado de hoy. Los ocho huecos se cerraron con tests, ninguno bajando la afirmación. |

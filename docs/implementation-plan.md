...
- ✅ Suspender/banear revoca sesiones al instante — verificado end-to-end. El
  guard resuelve la identidad contra la base en cada request, así que un access
  token ya emitido deja de servir en el request siguiente, no en quince minutos.
  Revocar los refresh tokens no alcanza para eso, y era lo único que había.
...
| La suite se marca su propio ritmo                       | La API permite veinte requests por segundo por cuenta —un techo razonable para una persona, muy por debajo de lo que hace una suite a fondo. Ningún test se acerca solo; varios seguidos sobre la misma cuenta dentro del mismo segundo sí. El 429 no llega como una falla clara: llega como una respuesta con otra forma (`carts is not iterable`) o como un precio que no cambió porque la escritura anterior fue rechazada, y el test que lo reporta casi nunca es el que lo causó. Esperar es el arreglo honesto: subir el límite para los tests sería dejar de correr contra los guards que la suite existe para probar. Va en un fixture y no en cada archivo porque el primer intento fue lo segundo, pacé los dos que se habían roto, y la corrida siguiente rompió otros dos. |
